...
def clean_data(df):
'''
Takes the raw df , manipulates and cleans the data by spreading out results under single column (categories) into multiple columns, removing unnecessary characters, redefining data types of new columns and dropping the non-required columns. Further concatenates the results and drops duplicates
...
'''
    ...
    df.to_sql('disaster_response_local_name', engine, index=False)
    engine.dispose()
    #pass  
...
'''
Specifying the program input in the format of messages_filepath, categories_filepath, db_filepath in command terminal runs the main program
